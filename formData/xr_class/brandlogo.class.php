...
class brandlogo {
    ...
    function change_status($id,$status){
        if (!$id) {
            ...
            $return = $db->update($setsql,"corporative_brand","id='{$id}'");
            if ($return) {
    ...
    function add($title,$image){
    	if (!$title) {
    		...
    		global $db;
    		$sql = "select title from corporative_brand where title='{$title}' limit 1";
    		$arr = $db->getone($sql);
    		if ($arr['title']) {
	            $arr_result['code'] = "error";
                ...
                $setsql['title'] = $title;
                $setsql['image'] = $image;
                $newid = $db->insert($setsql,"corporative_brand");
                if ($newid) {
                    $arr_result['code'] = "success";
    ...
    function delete($id){
        global $db;
        if ($id) {
            $sqls = array("delete from corporative_brand where id ='{$id}'");
    ...
    function get_limit($limit=0){
    	...
    	$sql = "select id,title,image from corporative_brand where status=1 ".$sql_limit;
    	$arr = $db->getall($sql);
    	$arr['logo_path'] = $config['dir_base_path'].$config['brand_logo']."/".$arr['image'];
    	return $arr;
    }
    ...
    function get_all_page($keywords,$sort,$page,$page_per_num=0){
        ...
        $total     = $db->getResultRows("select id from corporative_brand ".$sql_where );
        ...
        $sql_str = "select id,title,image,status from corporative_brand ".$sql_where.$sql_sort." limit ".$cur_pos.",".$per_page;
        ...
        for ($i=0;$i<(count($arr_str));$i++){
            ...
            $info[$i]['imagepath'] = $config['dir_base_path'].$config['brand_logo']."/".$arr_str[$i]['image'];
