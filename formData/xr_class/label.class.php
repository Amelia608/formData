...
class label {
    ...
    function get_one($id){
    	global $db;
    	$sql = "select title,status from news_labels where id='{$id}'";
    	$arr = $db->getone($sql);
    	return $arr;
    }
    ...
    function get_all(){
        global $db;
        $sql = "select id,title from news_labels where status=1";
        $arr = $db->getall($sql);
        return $arr;
    }
    ...
    function get_all_page($keywords,$sort,$page){
        ...
        $total     = $db->getResultRows("select id from news_labels ".$sql_where );
        ...
        $sql_str = "select id,title,status from news_labels ".$sql_where.$sql_sort." limit ".$cur_pos.",".$per_page;
    ...
    function add($title,$status){
        ...
        if (!$title) {
            ...
            global $db;
            /* 校验是否重复 */
            $arr_check = $db->getone("select title from news_labels where title = '{$title}'");
            if ($arr_check['title']) {
                $arr_result['code'] = "error";
                ...
                $newid = $db->insert($setsql,"news_labels");
                if ($newid) {
    ...
    function modify($id,$title,$status){
        ...
        if (!$id) {
            ...
            /* 校验是否重复 */
            $sql_check = "select title from news_labels where id != '{$id}' and title='{$title}'";
            ...
            if ($arr_check['title']) {
                ...
                $setsql['title']  = $title;
                $setsql['status'] = $status;
                $datareturn = $db->update($setsql,"news_labels","id='{$id}'");
                if ($datareturn=="0") {
                    $arr_result['code'] = "success";
    ...
    function change_status($id,$status){
        if (!$id) {
            ...
            unset($setsql);
            $setsql['status'] = $status;
            $return = $db->update($setsql,"news_labels","id='{$id}'");
            if ($return) {
                $arr_result['code'] = "success";
                $arr_result['info'] = "修改完成";
            }else{
