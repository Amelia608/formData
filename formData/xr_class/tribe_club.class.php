...
class tribe_club {
    ...
    function __construct(){
        $this->tbl_base               = "tribe_club";
        $this->tbl_label              = "tribe_club_labels";
        $this->tbl_case               = "tribe_club_cases";
        $this->tbl_case_comment       = "tribe_club_case_comment";
        $this->tbl_activities         = "tribe_club_activities";
        $this->tbl_activities_comment = "tribe_club_activities_comment";
        $this->tbl_login_log          = "tribe_club_login_log";
    }
    ...
    function get_one($id){
        global $db,$config;
        $sql = "select title,username,cover,baseinfo,synopsis,status,label,banner,logo from {$this->tbl_base} where id='{$id}'";
        $arr_tmp = $db->getone($sql);
        $arr_tmp['photo_path']        = $config['dir_base_path'].$config['tribe_club']."/".$arr_tmp['cover'];
        $arr_tmp['banner_path']       = $config['dir_base_path'].$config['tribe_club']."/".$arr_tmp['banner'];
        $arr_tmp['logo_path']         = $config['dir_base_path'].$config['tribe_club']."/".$arr_tmp['logo'];
        $arr_tmp['photo_path_small']  = $config['dir_base_path'].$config['tribe_club']."/small_".$arr_tmp['cover'];
        $arr_tmp['banner_path_small'] = $config['dir_base_path'].$config['tribe_club']."/small_".$arr_tmp['banner'];
        $arr_tmp['logo_path_small']   = $config['dir_base_path'].$config['tribe_club']."/small_".$arr_tmp['logo'];
        return $arr_tmp;
    }
     ...
     */
    function add($title,$image,$banner,$logo,$username,$password1,$password2,$label,$baseinfo,$synopsis,$status){
        if (!$title) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "请输入俱乐部名称";
        }elseif (!$image) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "请上传列表封面";
        }elseif (!$banner) {
            $arr_result['code'] = "error";
            ...
            $arr_result['code'] = "error";
            $arr_result['info'] = "请填写俱乐部信息";
        }elseif (!$label) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "请选择标签";
        // }elseif (!$synopsis) {
        //     $arr_result['code'] = "error";
        //     $arr_result['info'] = "请填写简介";
            ...
            if ($temp1['username']) {
                ...
                $arr_result['code'] = "error";
                $arr_result['info'] = "俱乐部名称已经存在";
            }else{
                unset($setsql);
                $setsql['title']         = $title;
                $setsql['cover']         = $image;
                $setsql['banner']        = $banner;
                $setsql['logo']          = $logo;
                $setsql['label']         = $label;
     ...
     * @return [type] [description]
     */
    function modify($id,$title,$image,$banner,$logo,$username,$password1,$password2,$label,$synopsis,$baseinfo,$status){
        if (!$id) {
            $arr_result['code'] = "error";
     ...
     * @return [type] [description]
     */
    function change_status($id,$status,$club_id=0){
        if (!$id) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "信息丢失";
        }else{
     ...
     * @return [type] [description]
     */
    function get_one_case($id,$showcategore=1,$club_id=0){
    	global $db,$config;
        ...
        if ($arr['image']) {
            $arr['photo_path'] = $config['dir_base_path'].$config['tribe_club']."/small_".$arr['image'];
        }
    	return $arr;
    }
     ...
     * @return [type] [description]
     */
    function get_case_page($keywords,$sort,$page,$club_id=0){
        global $db,$config,$page_fn;

        if ($page=="" or $page<=0) $page=1;

        if ($club_id) {
            $sql_where=" where cate_id='{$club_id}'";
        }else{
            $sql_where=" where 1=1";
        }
        ...
        $sql_str = "select id,cate_id,title,pageview,reviewtimes,status,addtime from ".$this->tbl_case.$sql_where.$sql_sort." limit ".$cur_pos.",".$per_page;
        $arr_str = $db->getall($sql_str);
     ...
     * @return [type] [description]
     */
    function change_case_status($id,$status,$club_id=0){
        if (!$id) {
            $arr_result['code'] = "error";
     ...
     * @return [type] [description]
     */
    function delete_comment($id,$club_id=0){
        global $db;
     ...
     * @return [type] [description]
     */
    function get_comment_page($keywords,$sort,$page,$cate_id=0,$club_id=0){
        global $db,$config,$page_fn;
     ...
     * @param [type] $content [description]
     */
    function add_review($commentid,$usertype,$userid,$content,$club_id=0){
        if (!$commentid) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "信息丢失";
        }elseif (!$userid) {
            ...
            global $db;
            if ($club_id) {
                $sql_where = " id='{$commentid}' and cate_id='{$club_id}'";
     ...
     * @return [type] [description]
     */
    function get_activities_page($keywords,$sort,$page,$club_id=0){
        global $db,$config,$page_fn;
     ...
     * @return [type] [description]
     */
    function get_one_activity($id,$showcategore=1,$club_id=0){
        global $db,$config;
        if ($club_id) {
            $sql_where = " and cate_id='{$club_id}'";
        }
     ...
     * @return [type] [description]
     */
    function get_comment_page_activity($keywords,$sort,$page,$cate_id,$club_id=0){
        global $db,$config,$page_fn;
     ...
     * @return [type] [description]
     */
    function delete_comment_activity($id,$club_id=0){
        global $db;
        // $sql = "update {$this->tbl_activities_comment} set comment='评论已删除' where id='{$id}'";
        if ($club_id) {
            $sql_where = " id='{$id}' and brand_id='{$club_id}'";
        }else $sql_where = " id='{$id}'";
     ...
     * @param [type] $content [description]
     */
    function add_review_activity($commentid,$usertype,$userid,$content,$club_id=0){
        if (!$commentid) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "信息丢失";
        }elseif (!$userid) {
            ...
            global $db;
            if ($club_id) {
                $sql_where = " id='{$commentid}' and brand_id='{$club_id}'";
            }else $sql_where = " id='{$commentid}'";
        ...
        if (!$username) {
            ...
            if ($arr_adm['title']){
                ...
                //     $needLog = "y";
                    ...
                    $_SESSION['usertype']    = "俱乐部";
                    $_SESSION['club_userid'] = $arr_adm['id'];
                    $_SESSION['club_title']  = $arr_adm['title'];
                    $_SESSION['club_name']   = $arr_adm['username'];
     ...
     * @param [type] $content [description]
     */
    function add_baseinfo($userid,$title,$synopsis,$cover,$banner,$logo,$content,$label){
        if (!$userid) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "用户信息丢失";
        }elseif (!$title) {
            $arr_result['code'] = "error";
            $arr_result['info'] = "请填写俱乐部名称";
        // }elseif (!$synopsis) {
        //     $arr_result['code'] = "error";
        //     $arr_result['info'] = "请填写简介";
        }elseif (!$cover) {
        ...
        //     $arr_result['info'] = "请输入俱乐部介绍";
            ...
            $setsql['logo']     = $logo;
            $setsql['synopsis'] = $synopsis;
            // $setsql['baseinfo'] = $content;
            $newid = $db->update($setsql,"{$this->tbl_base}"," id='{$userid}'");
            if (!$newid) {
                $arr_result['code'] = "error";
                $arr_result['info'] = "数据操作失败";
            }else{
